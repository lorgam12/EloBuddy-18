...
namespace SATANIXKatarina
{
    ...
    {
        public static Menu ComboMenu, HarassMenu, FarmMenu, JungleMenu, HumanizerMenu;
        ...
        public static void Init()
        {
            ...
            HumanizerMenu = Program.menu.AddSubMenu("Humanizer");
            HumanizerMenu.Add("Humanizer", new CheckBox("Use Humanizer?", false));
            HumanizerMenu.Add("QDelay", new Slider("Q Delay (ms)", 0, 0, 500));
            HumanizerMenu.Add("WDelay", new Slider("W Delay (ms)", 0, 0, 500));
            HumanizerMenu.Add("EDelay", new Slider("E Delay (ms)", 0, 0, 500));
            HumanizerMenu.Add("RDelay", new Slider("R Delay (ms)", 0, 0, 500));
		...
		private static void Combo1() //QEW
		{
            ...
            {
                if(HumanizerMenu["Humanizer"].Cast<CheckBox>().CurrentValue)
                    Core.DelayAction(() => Program.Q.Cast(target), HumanizerMenu["QDelay"].Cast<Slider>().CurrentValue);
                else
            ...
            if (Program.E.IsReady() && _Player.Distance(target.Position) < Program.E.Range && !Program.Q.IsReady())
            {
                ...
                if (HumanizerMenu["Humanizer"].Cast<CheckBox>().CurrentValue)
                    Core.DelayAction(() => Program.E.Cast(target), HumanizerMenu["EDelay"].Cast<Slider>().CurrentValue);
                else
        ...
        public static void Combo()
        {
            ...
            if (target.IsValidTarget())
            {
                ...
                else
                {
                    ...
                    {
                        if (HumanizerMenu["Humanizer"].Cast<CheckBox>().CurrentValue)
                            Core.DelayAction(() => Program.W.Cast(), HumanizerMenu["WDelay"].Cast<Slider>().CurrentValue);
                        else
                            Program.W.Cast();
                    }
                        ...
                        _Player.CountEnemiesInRange(Program.R.Range) > 0)
                    {
                        if (!Program.Q.IsReady() && !Program.E.IsReady() && !Program.W.IsReady())
                        {
                            ...
                            Orbwalker.DisableMovement = true;
                            if (HumanizerMenu["Humanizer"].Cast<CheckBox>().CurrentValue)
                                Core.DelayAction(() => Program.R.Cast(), HumanizerMenu["RDelay"].Cast<Slider>().CurrentValue);
                            else
                                Program.R.Cast();
